...
(function(){
  ...
  // scanner handling (record_attendance.php) - trigger automatically when input reaches 10 characters
  if (scanInput) {
    let processingScan = false;
    // centralize processing so both Enter and length trigger use same logic
    function processScan(nis) {
      nis = String(nis || '').trim();
      if (!nis) {
        setFeedback(scanFeedback,'Tidak ada data ter-scan.','error');
        return;
      }
      if (processingScan) {
      ...
      fetch('record_attendance.php', {
        ...
        if (data && data.success) {
          ...
          if (data && data.server_text) {
            const snippet = data.server_text.length > 300 ? data.server_text.substring(0,300) + '...' : data.server_text;
            setFeedback(scanFeedback,'Server response tidak valid: ' + snippet, 'error');
            console.error('record endpoint non-JSON response:', data.server_text);
          } else {
    ...
    // When scanner sends Enter (most USB HID barcode scanners do), keep existing behavior
    ...
    // New: when input length becomes exactly 10 characters, trigger automatically.
    // This handles scanners that paste characters without Enter.
    scanInput.addEventListener('input', function(e){
      const val = scanInput.value || '';
      // remove surrounding whitespace
      const trimmed = val.trim();
      if (trimmed.length === 10) {
        // small timeout to allow scanner to finish sending characters if any buffering
        setTimeout(()=> {
          // double-check length again
          if ((scanInput.value || '').trim().length === 10) {
            processScan(trimmed);
          }
        }, 20);
      }
